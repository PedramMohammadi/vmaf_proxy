...
if __name__ == "__main__":
    ...
    parser.add_argument("--norm_groups", type=int, default=32, help="Groups per conv block. Ideally a divisor of the layer's channel count.")
    ...
    parser.add_argument("--activation", type=str, default="relu", choices=["relu", "leaky_relu"], help="Activation function in the model ('relu' or 'leaky_relu'; default: 'relu').")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for ref, dist, target in tqdm(train_loader, desc=f"Epoch {epoch+1}/{args.epochs} - Training"):
            ref, dist, target = ref.to(args.device), dist.to(args.device), target.to(args.device)

            optimizer.zero_grad()
            prediction = model(ref, dist)
        ...
        with torch.no_grad():
            for ref, dist, target in tqdm(val_loader, desc=f"Epoch {epoch+1}/{args.epochs} - Validation"):
